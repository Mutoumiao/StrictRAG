/**
 * L2 多轮题面解析（纯函数，无 fs）。
 * 文件账本仍是 fixtures/l2/gold.yaml；禁止塞进 gold_questions。
 */
...
export function parseL2Gold(data: unknown): L2GoldFile {
...
function parseCase(raw: unknown, index: number, seen: Set<string>): L2Case {
  ...
  if (typeof type !== 'string' || !L2_TYPE_SET.has(type)) {
    throw new L2GoldLoadError(`cases[${index}].type must be one of ${L2_TYPES.join('|')}`);
  }
...
function parseTurns(raw: unknown, index: number, type: L2Type): L2Turn[] {
  ...
  for (let j = 0; j < raw.length; j++) {
    ...
    if (typeof row.session !== 'string' || !L2_SESSION_SET.has(row.session)) {
      throw new L2GoldLoadError(`cases[${index}].turns[${j}].session must be same|new|none`);
    }
