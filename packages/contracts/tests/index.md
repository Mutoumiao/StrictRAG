...
| `async/ingest-job.test.ts` | 入库任务 DTO 必须覆盖全阶段，拒绝空 docId 与非法 stage；逻辑 stage 隔离。 | prds/06-async · 剧本 Q5 | `IngestJobDataSchema · INGEST_STAGES · INGEST_JOB_DEFAULT_ATTEMPTS · QUEUE_NAMES` | 含逻辑 stage ocr / purge；物理队列仍单条 sr-ingest、不与 stage 重名；stage 互不重复且各自可解析。 | 现行 |
...
| `eval/l1-hit-at-k.test.ts` | 有 expectedDocIds 的 L1 题必须按 evidence 交集计 Hit@k，无名单不计分。 | 覆盖 C4 · prds/08-quality | `hitAtKCase · accumulateHitAtK · hitAtKRate · parseExpectedDocIds` | 不进 2×2 / signoffEligible；脏名单抛错。逻辑 id→uuid 映射层只认 trim 后全等，未映射不计命中。 | 现行 |
...
| `ingest/context-mode.test.ts` | L0 模板必须用标题（有路径才拼接）；非法 contextMode 不得当已实现 L1；可落库来源含 `l1_llm`。 | 入库 PRD §4 · 功能表 §4.5 / §6 | `parseContextMode · l0ContextPrefix · resolveContextSource · CONTEXT_SOURCES · invalidContextModeOverride` | `resolveContextSource(l1_llm)` 仍是 `l0_fallback`（是否真 L1 由 worker 决定）；`l1_llm` 只在真调通时写。 | 现行 |
...
| `ingest/document-contract.test.ts` | 文档 / 知识库 DTO 与完成上传、补丁元数据、ACL 专用入口必须接受合法部门可见级与名单三态并拒非法值；收紧判定须可核对。 | 入库 HTTP · prds/05-api §2.4 · ADR-009 决策 4 | `CreateKbBodySchema · KnowledgeBaseListItemSchema · VisibilityLevelSchema · CompleteUploadBodySchema · WriteDocumentBodySchema · WriteDocumentResponseSchema · PatchDocumentMetaBodySchema · DocumentAclSchema · PutDocumentAclBodySchema · PutDocumentAclResponseSchema · aclTightens · DocumentDetailSchema · DocumentListItemSchema · ReindexDocumentResponseSchema · SupersedeDocumentBodySchema · SupersedeDocumentResponseSchema · DeleteDocumentResponseSchema` | 文档 DTO 含列表 docType / aclPrincipals / 生效区间 / 替代边；complete / PATCH 可写名单三态；ACL 端点 GET/PUT 形状（`.strict()`，超 256 拒）；PUT 响应带 `reindexRequired`；`aclTightens` 真值表（null→[] 收紧、[]→null 放宽、[a]→[b] 收紧）；PATCH 可写本地时间串窗口；complete 可选 64 位 checksumSha256；write 须 title+markdown 且 sourceType=write；reindex stage 可 chunk 或 ocr；supersede 须 successorDocId；DELETE 响应 archived + purgeEnqueued。 | 现行 |
...
| `ingest/ingest-report-contract.test.ts` | 入库报告 DTO 须含跨 doc skip、情境来源、去重率（0–1 或 null）与 contextualize 两计数（可空整数），仍拒绝 Hit@k / 未知字段。 | prds/05-api GET ingest-report · 功能表 §5.2 · 入库 PRD §4 / §5.2 | `IngestReportItemSchema` | 跨文档去重 + contextSource + dedupeCrossDocRate + `contextualizeL1Ok` / `contextualizeL0Fallback`；不是 pending_review。 | 现行 |
| `kb/cross-doc-dedupe-action.test.ts` | KB 设置 PATCH 只收 skip_index / pending_review；PRD 的 downrank 必须被拒。 | 剧本 E4 · 入库 PRD §5.1 | `PatchKbSettingsBodySchema / parseCrossDocDedupeAction` | 两合法值通过；downrank 与未知值拒；读取缺省/脏值回落 skip_index。 | 现行 |
