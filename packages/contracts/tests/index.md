...
| `kb/settings-contract.test.ts` | KB 设置 PATCH 仅白名单且拒阈值字段，GET 必须锁定 rewrite 关闭；成员档位口不得夹带 τ。 | B2 · 功能表 §3 问答档位 · ADR-050 | `PatchKbSettingsBodySchema · KbSettingsSchema · AskModesSchema · KbDocTypesSchema` | KB 设置形状与 sessionRewrite 锁定；ask-modes 仅 allowedModes/defaultMode；doc-types 仅 items。 | 现行 |
