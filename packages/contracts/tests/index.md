...
| `async/ingest-job.test.ts` | 入库任务 DTO 必须覆盖全阶段，拒绝空 docId 与非法 stage。 | prds/06-async · 剧本 Q5 | `IngestJobDataSchema · INGEST_STAGES · INGEST_JOB_DEFAULT_ATTEMPTS` | 含逻辑 stage ocr；物理队列仍 sr-ingest。 | 现行 |
