...
| `ingest/ingest-report-contract.test.ts` | 入库报告 DTO 须含跨 doc skip 事实，仍拒绝 Hit@k / 未知字段。 | prds/05-api GET ingest-report · 功能表 §5.2 · 入库 PRD §5 | `IngestReportItemSchema` | 跨文档去重最小闭环形状；不是 pending_review。 | 现行 |
