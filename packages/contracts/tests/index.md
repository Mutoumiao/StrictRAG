...
| `ingest/ingest-report-contract.test.ts` | 入库报告 DTO 须含跨 doc skip 与情境来源，仍拒绝 Hit@k / 未知字段。 | prds/05-api GET ingest-report · 功能表 §5.2 · 入库 PRD §4 / §5 | `IngestReportItemSchema` | 跨文档去重 + contextSource；不是 pending_review / 不是 L1 成功。 | 现行 |
