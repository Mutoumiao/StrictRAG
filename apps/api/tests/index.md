...
| `eval/http-eval-runs.test.ts` | POST eval/runs 只入队；空题集拒绝；L2 不依赖 gold_questions；内口靠口令并可带窗。 | prds/05-api §2.8 · 功能表 §5.2 | `POST/GET …/eval/runs · POST /internal/eval/execute-ask` | 请求线程不跑批。 | 现行 |
