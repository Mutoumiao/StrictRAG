...
| `acl/doc-read-kb-member-gate.test.ts` | 路径只有 :docId 的文档读入口必须查 KB 成员资格，持码非成员不得读他库元数据 / ACL 名单 / 分片正文。 | ADR-035 §决策 4 · ADR-057 决策 2 · prds/09-security §3.2 | `GET /documents/:docId · …/acl · …/ingest-jobs · …/chunks · …/chunks/:chunkId` | 非成员 403 非成员文案且数据仓零调用；成员 200；super_admin 非成员旁路；`chunk.view` 两入口硬姿态、`doc.view` 三入口姿态随 AUTH_ENFORCE。 | 现行 |
