...
| `ops/kb-settings-services.test.ts` | 设置服务必须把类型分区草稿编成 catalog，失败则 PATCH 写出错误 docTypeItems。 | 功能表 §4.2 文档类型 · ADR-054 · 工单「类型分区 CRUD 最小闭环」 | `draftsFromSettings / draftsToCatalog / catalogsEqual` | 不写 URL；不再把逗号串当主路径。 | 现行 |
