...
| `ops/documents-workspace.test.tsx` | 文档列表薄页必须按码控制详情/保存/部门列，失败则运营交互与权限不符。 | 文档运营 UI | `DocumentsWorkspace · deptLabel / readyColLabel / visibilityLabel` | 含类型列、运营标签、Reindex、归档、在线编写区；行展开名单与生效区间；PATCH 含 effectiveFrom/To；有码可选后继替代。 | 现行 |
