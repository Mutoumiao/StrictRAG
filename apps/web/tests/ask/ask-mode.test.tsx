...
describe('AskPanel 档位', () => {
  ...
  it('读 allowedModes，默认 defaultMode，不展示未允许档', async () => {
    ...
    const trigger = screen.getByLabelText('问答档位');
    expect(trigger.tagName).toBe('BUTTON');
    expect(trigger).toHaveTextContent('快速（fast）');
    await user.click(trigger);
    expect(screen.getByRole('option', { name: /快速/ })).toBeInTheDocument();
    expect(screen.getByRole('option', { name: /严谨/ })).toBeInTheDocument();
    expect(screen.queryByRole('option', { name: /均衡/ })).not.toBeInTheDocument();
    await user.click(screen.getByRole('option', { name: /严谨/ }));
    expect(screen.getByLabelText('问答档位')).toHaveTextContent('严谨（strict）');
  });
});
