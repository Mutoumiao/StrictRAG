...
| `ingest/es-builder-tenant-required.test.ts` | 无 tenantId 的 ES bulk builder 必须失败，且不得发出任何 HTTP。 | 剧本 O4 · ADR-041 | `sparseBulkSource / bulkIndexSparse` | 缺 / 空 tenantId 构 source 即抛；抛之前不 fetch；带 tenantId 时 source 逐位不变。 | 现行 |
