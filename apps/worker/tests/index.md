...
| `eval/run-l1-batch.test.ts` | worker L1 批跑必须串行入 2×2，error 出格。 | prds/08-quality §2 · 覆盖 C4 · 覆盖 C2 | `runL1Batch` | 注入 execute；有 expectedDocIds 计 Hit@k；有 minSupport 计 tauStar；≠ 签字 PASS。 | 现行 |
