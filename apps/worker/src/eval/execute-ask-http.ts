...
import type { L2TurnExecute, L2TurnExecuteResult } from './run-l2-batch.js';
...
export function createEvalHttpL2Execute(opts: EvalHttpExecuteOpts): L2TurnExecute {
  const fetchImpl = opts.fetchImpl ?? fetch;
  const base = opts.baseUrl.replace(/\/$/, '');

  return async ({ question, sessionId, sessionWindow }): Promise<L2TurnExecuteResult> => {
    if (!opts.token) {
      return { outcome: 'error', errorMessage: 'EVAL_INTERNAL_TOKEN is empty' };
    }
    const res = await fetchImpl(`${base}/api/v1/internal/eval/execute-ask`, {
      ...
      body: JSON.stringify({
        ...
        userId: opts.userId,
        question,
        ...(sessionId ? { sessionId, sessionWindow } : {}),
      }),
    });
    let payload: ExecuteAskJson = {};
    try {
