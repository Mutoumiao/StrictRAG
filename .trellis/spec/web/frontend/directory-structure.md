...
apps/web/
  ...
  src/
    ...
    api/                      # 业务 HTTP（界面简单 → 集中）
      ...
      sessions.ts
      feedback.ts
      knowledge-bases.ts      # GET 身份可见库（ask 原生下拉只消费本次行）
    services/
...
| services | **现状** `src/services/*.services.ts`（例：`sessions.services.ts`） | 用例编排（调 api、toast、store） | **后端 URL/path** |
...
3. **services**（`src/services/*`）**禁止写请求 URL**；只调 api 函数。体量大时按用例拆更多 `*.services.ts`。域多且独立后再考虑 `features/`（见 module-layering；**禁止**为对称空建 features）。  
...
10. **测试**：现行 `tests/<能力>/`；`src/test/` **只** setup / re-export；ask final 工厂权威在 `@strict-rag/contracts/testing`（`src/test/fixtures/ask.ts` 仅 re-export）；禁止业务 import `*.test.*`；E2E 不进 `src/`。HOW：[testing](../../guides/testing.md) · 导航 `tests/index.md` · P0 `docs/testing/p0-redlines.md`。
