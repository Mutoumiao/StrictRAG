...
apps/api/
  ...
  src/
    ...
    routes/
      ...
      ingest-report.ts       # GET …/ingest-report（库级已落库行含跨 doc 冲突对；doc.view WhenEnforced + 成员闸）
      ...
      kb-settings.ts         # B2/B2-W 知识库设置（含类型分区 catalog）+ GET settings-audit + KB 消费绑定 PUT（只 generate/embed/rerank）
