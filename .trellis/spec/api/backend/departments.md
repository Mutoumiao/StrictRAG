...
> 切片：**最小**（树 CRUD + 用户归属）；grant 表可存；`DEPT_ACL_ENFORCE` **默认关**；开时精确 ∪ 祖先 + 精确 grant；超管可绕过；列表同滤且列表项带部门列；`DEPT_INHERIT_DOWN` 默认 true；KB `deptInheritDown` 可覆盖 env；complete 可写部门字段；**≠** ES 查询期 / **≠** 默认开 / **≠** 解禁 / **≠** 设置页 inherit 勾选
...
- `DEPT_ACL_ENFORCE` 默认 false；开时 `filterDocsForDeptAcl`：精确 ∪ 祖先 + 精确 grant（预览、列表、retrieve 同函数）；`roleBypassesKbMembership` 绕过部门滤（Pino `dept_acl_bypass`）；`DEPT_INHERIT_DOWN` 默认 true（仅 `'false'` 关祖先）；KB `config_json.deptInheritDown` 可覆盖 env（未写跟 env）；**无** ES 查询期对称 / **无** 默认开 / **无** 设置页 inherit 勾选
