...
> **SSOT**：实现以本表 + 源码为准；PRD 多跳/CRAG **未**实现；加深仅 session 显式回溯；document 只加码检索；禁止假画 intent LLM / external。
...
| CRAG grade → refine | **无** |
| 按需加深（窗外再取 K 条） | **session 加深已落**（`isExplicitSessionBackref` + `clipSessionWindow({ deepened })` 硬顶 8）；**document 加码已落**（`isExplicitDocumentBackref` + 末轮 `docId` → `promotePreferredDocChunks`，不翻聊天）；**无** intent LLM / **无** external / **无**二次加深 |
...
| L3 打点 / 自动熔断 | **打点有**（`recordL3Ask` 五键，含 `l3_document_backref_total`）；**自动熔断 / 面板无**（见 [l3-metrics](./l3-metrics.md)） |
...
| `debug` | 仅 `options.debug=true`；`rewriteUsed` / `sessionDeepened` / `documentBackref` **跟图**（关时仍 false）；可含 `standaloneQuestion` |
