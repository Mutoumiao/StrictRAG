...
- [ ] 触及按需加深 / `sessionDeepened` / `documentBackref` / `externalBackref` 时是否读 [ask-pipeline](./ask-pipeline.md)（session 加深 + document 加码 + external 抑制已落；禁 intent LLM / 默认改窗）？
...
- [ ] 触及 L3 打点/告警时是否读 [l3-metrics](./l3-metrics.md)（打点+告警 ≠ 准出 / ≠ 熔断；**禁止**按计数或告警改默认）？  
...
| [l3-metrics](./l3-metrics.md) | **P2.5-L3/L3A** 多轮护栏打点+告警 · **≠** 准出 / **≠** 自动熔断 / **≠** 面板 |
