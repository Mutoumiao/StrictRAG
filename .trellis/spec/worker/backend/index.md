...
> 现状：probe + ingest 状态机（scan→parse→ocr?→chunk→embed→es）；默认 `INGEST_SCAN_MODE=mock_clean` · `INGEST_ES_MODE=mock` · `INGEST_OCR_ENABLED=false`；可显式 `http`/S3/Mongo（OPS-STACK）；**QUAL-2 真杀毒 = 安全债**（DEC-SCAN，非本进程「已完成」）。
