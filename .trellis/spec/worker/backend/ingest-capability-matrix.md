...
| **ingest_reports** | **partial** | `ingest-report.ts` + pipeline + `cross-doc-dedupe.ts` | 双就绪 / 文档内去重清空 / 同 KB 跨 doc skip_index 冲突对 / `contextSource` l0\|l0_fallback / 对账失败落可查询行 | pending_review / downrank / 生产 LSH / Hit@k / l1_llm 成功计数 |
...
| **PG `ingest_reports`** | **worker** `ingest-report.ts` | api GET 库级报告 | 双就绪 / 文档内+跨 doc skip / 对账失败 | 不含 pending_review / L1 / Hit@k |
