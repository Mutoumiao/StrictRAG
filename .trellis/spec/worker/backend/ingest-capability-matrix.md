...
| **ingest_reports** | **partial** | `ingest-report.ts` + pipeline + `cross-doc-dedupe.ts` | 双就绪 / 文档内去重清空 / 同 KB 跨 doc skip_index 冲突对 / `contextSource` l0\|l0_fallback\|**l1_llm** / 对账失败落可查询行 / **`dedupeCrossDocRate`**（`crossDocDropped / 三者之和`；分母 0 → null，**不写 0**）/ **`contextualizeL1Ok` + `contextualizeL0Fallback`**（PRD 04 §5.2「指标必出」；与 `contextSource` 同口径——L1 被请求但未实际调用时整轮计回退；后阶段不得复写） | pending_review / downrank / 生产 LSH / Hit@k / 「高度重复」阈值提示（数据 PRD 无阈值） |
...
| **PG `ingest_reports`** | **worker** `ingest-report.ts` | api GET 库级报告 | 双就绪 / 文档内+跨 doc skip / 跨文档去重率 / 对账失败 | 不含 pending_review / L1 / Hit@k |
...
| **ES 稀疏索引** | **worker** `es_index` | api `RETRIEVE_ES_MODE=http`（可选） | worker：默认 `mockEsStore`；`INGEST_ES_MODE=http` 写 mapping/bulk（含可选 `ownerDeptId` / `aclPrincipals`） | 默认 mock；http ≠ IK / Router |
