...
| 失败 Webhook | **最小已** | `failure-webhook.ts`：`recordStageEnd` 见 `errorCode` 时 POST JSON；`INGEST_FAILURE_WEBHOOK_URL` 空则不发；~3s 只一次；非 2xx/网络错 warn 不抛；**无** HMAC / 重试队列 / ask webhook |
| 同 doc 并发锁 | **最小已** | worker `doc-lock.ts`：Redis `SET NX EX` + token 安全释放；`index.ts` 持锁跑 stage；`DOC_LOCK_BUSY` 可重试；**非** Redlock / 多 master |

**实现 task**：`08-12-spec-w1-ingest-idempotency-impl`（幂等）· `08-12-ingest-jobs-ledger-min`（账本最小）· `08-12-ingest-doc-lock-min`（锁最小）。  
**禁止**：把「最小幂等/最小账本/最小锁/最小失败 Webhook」写成「生产级重试/账本/Redlock/HMAC 死信已齐」。
