...
> 现状：**S2c 薄运营页**（登录 · 壳 · 文档/审批/成员 + **B1–B6** + **B13 反馈队列**）；文档列表带部门列（有树显示名、可见级默认中文标签）、有 manage 可下拉、可本地筛；设置页可标 `dataClass` 并可勾选 inherit（未改不写回；**≠** 解禁 / **≠** 打开强制）；grant 可选部门+用户（无 `user.manage` 用户仍 uuid）·归属可选部门；**非**完整运营台（**无** APM 时序 / **无** DEPT_ACL 强制 UI）。
