...
> 现状：**S2c 薄运营页**（登录 · 壳（当前 KB 关闭列表只列本次 GET 可见库，禁止粘贴 uuid）· 文档/审批（回显提交人）/成员 + **B1–B6** + **B13 反馈队列**）；文档列表带部门列、类型列、运营标签、Reindex、四态 lifecycle（有树显示名、可见级默认中文标签）、**创建面 ClosedSelect 标新文档部门/可见级**、**后继替代关闭列表**、有 manage 可下拉、可本地筛、可扫向量/稀疏就绪（**≠** 生产 ES）；文档行展开可回读/编辑 `aclPrincipals`（Textarea uuid，逗号或换行；勾选「仅名单可见」；有 `doc.editor` 才保存；**≠** 用户下拉 / **≠** 角色 principal）；设置页 **文档类型分区**（code / 显示名 / 排序 / 启用；**非**逗号串）、可标 `dataClass`（complete 须 ACL 就绪，**≠** 仓库默认开 / **≠** 角色 principal）、可勾选 inherit（未改不写回）与 **本库强制**（未改不写回；**≠** 仓库默认开）、页内 **修改日志**（时间 / 操作者 / 字段旧→新；无行「暂无修改日志」；**无**独立路由）；grant 可选部门+用户（无 `user.manage` 用户仍 uuid；行内部门名+可见级文案+过期「长期」/原串）·归属可选部门+用户；**非**完整运营台（**无** APM 时序 / **≠** 全文隔离 / **≠** ES）。
...
- [ ] 评测页是否只经本模块 `api.ts`、验 `eval.run`、**禁止**把覆盖率或 L2 工程绿当签字/准出 PASS？  
...
- [ ] 角色页授码是否按 `admin-catalog` 的 `MENU_TREE` 分组（L1 / L2 / 操作码）？任何 catalog 码是否都有落点（未挂菜单的码进「其他（未挂菜单）」，**禁止静默丢码**）？
