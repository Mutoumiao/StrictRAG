...
**What**：文档行展开区紧挨「入库阶段」展示本行入库报告。库级 `GET …/ingest-report` 后按 `docId` 过滤。无行显示「暂无入库报告」。只写真事（分片数 / 文档内 dropped / 双就绪 / 跨文档去重率 / 对账）；不造 Hit@k。**去重率未记录（`null`，分母为 0 的轮次 / 迁移前旧行）必须显示「未记录（本轮无参与去重的切片）」，禁止显示 `0.0%`** —— 那会把「没参与去重」说成「零重复」。
