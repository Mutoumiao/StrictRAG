...
> **Gotcha（实测 · 2026-09-20 更新）**：基线快照原先只剩 `0000_snapshot.json`，`db:generate` 会退化成把全部表按当前 schema 重写一遍全量 `CREATE TABLE`（**25 张**，`schema_meta` 已被 0000 覆盖）。**2026-09-20 已补 `0021_snapshot.json`**（`0001`–`0020` 仍缺，但 `generate` 只读排序末位快照，故已恢复可用；硬验收 = 打印 `No schema changes, nothing to migrate 😴`，**该验收在仓外副本达成、未在仓库工作区跑过**）。**注意**：恢复可用**不等于**改回 generate 流水线 —— 本仓当前有效实践仍是：**手写 migration SQL（`ALTER TABLE … ADD COLUMN IF NOT EXISTS`）+ 手写 `meta/_journal.json` 条目**（tag 用描述名，如 `0017_ask_traces_citations`、`0018_ingest_report_dedupe_rate`、`0019_ingest_report_contextualize_counts`、`0020_documents_active_index_version`、`0021_chunks_dedupe_review`，`idx` 递增、`when` 递增一天）；`db:generate` 输出只当对照。**禁止**提交全量 `CREATE TABLE` 的生成结果 —— 已装 `drizzle-kit@0.31.10` 的生成器**不输出 `IF NOT EXISTS`**，产物会让 `migrate` 在已建表的库上直接报 `relation "…" already exists`。

> **口径自检**（过期就改这一行，别写死印象）：以 `packages/db/drizzle/meta/*_snapshot.json` 的实际清单为准 —— `generate` 只读排序**末位**快照，所以只要末位快照与迁移链对齐，`generate` 就可用。`docs/module-status/db.md` 是同一事实的 IS 镜像，两处口径必须一致。补基线快照的工单与验收见 wayfinder `.scratch/p2-exit-evidence/issues/11-drizzle-meta-baseline.md`；取证与 A1 步骤见 `.scratch/fill-must-haves/research-drizzle-meta-baseline.md`（注意该文编号已过期）。
