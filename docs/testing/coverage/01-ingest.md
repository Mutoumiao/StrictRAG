...
| E4 | 跨 doc 近重复：指标可见；pending_review 可人工处理 | 源码为准 | 单测 | 部分测 | worker | `cross-doc-dedupe.test.ts` · `cross-doc-skip-index.test.ts`：同 KB Jaccard≥0.9 skip_index + 报告冲突对；archived/跨 KB 不比 | **无** pending_review 人工二选一；**无** 生产 MinHash LSH |
| E5 | L1 故障 → L0 回退仍 ready | P2必签 | 单测 | 部分测 | worker | `context-mode-obey.test.ts`：快照 `l1_llm` / 缺省走 L0 标题 prefix，报告 `l0_fallback`；`l0_template` 记 `l0` | **无** Gateway contextualize / 真 429 注入；**无** `context_source=l1_llm` |
