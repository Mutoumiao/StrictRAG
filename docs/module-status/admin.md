...
| 最近更新 | 2026-09-08（评测账本 L1 展示 Hit@k 与 tau*） |
...
### 评测底线
- `/eval`：黄金集增删 + 「跑一批」入队 L1 + 「跑 L2」入队 session_multiturn + 列出账本；L1 有 scored 时展示 Hit@k；有 tauStar 时展示该阈值（null 显示「无」）；需要 eval.run；工程可签字文案 ≠ 准出 PASS
