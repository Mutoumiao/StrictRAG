...
| 最近更新 | 2026-08-20（P3b-ENFUI 设置页可勾选强制、未改不写回；进程默认仍关；**≠** 强制已上 / **≠** ES 查询期 / **≠** 解禁） |
...
| 跨部门授权、DEPT_ACL 强制 | grant 可存可配；过滤默认关；开时精确 grant 进检索（不沿子树）；超管可绕过；列表同滤；可关继承；KB 可覆盖 enforce（未写跟 env；设置页可勾选，未改不写回）；ADR-057 全文未上（无 ES 查询期 / 无解禁 / 无默认开） |
