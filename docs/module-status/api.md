...
| 入库报告完整语义 | 库级 GET 含跨 doc skip 冲突对、**`dedupeCrossDocRate`** 与 **`contextualizeL1Ok` / `contextualizeL0Fallback`**（三者 null 均原样回读，不填 0）；**无** `pending_review` / L0 vs L1 Hit@k / 「高度重复」阈值提示 |
