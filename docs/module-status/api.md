...
| 最近更新 | 2026-09-20（路径只有 `:docId` 的 10 个文档写入口补 handler 级 KB 成员闸 `docWriteMemberDenied`：姿态随该入口权限码，super_admin 旁路；新增 `tests/acl/doc-write-kb-member-gate.test.ts`）；2026-09-19（剧本 E4 去重冲突 `resolve` 端点；ADR-046 签字包读时派生；O4 双仓 `tenantId` 契约闸；AA1 保文档断言）；2026-09-17（问答 `Idempotency-Key`：同 key 不重跑、终态复用、在途 409；入库报告回读 `contextualizeL1Ok` / `contextualizeL0Fallback`；`PUT /documents/:docId/acl` 收紧时回 `reindexRequired`）；2026-09-16（文档 ACL 专用端点 `GET/PUT /documents/:docId/acl`；上传体积族级档：MD/TXT 更严 10 MiB；指标骨架补 `fallback` / `node_used` 维；`GET /ask/:requestId/final` 断线重拉终态；`ask_traces.citations` 落库） |
...
- **路径只有 `:docId` 的文档写入口已补 handler 级 KB 成员闸**（`docWriteMemberDenied`，2026-09-20）：中间件只能从 path 取 `:kbId`，`PATCH/PUT /documents/:docId*` 这类入口原先只验权限码，持码非成员可跨库写。现覆盖全部 **10 个写入口**：重跑入库、审批通过、驳回、扫描入队、lifecycle、替代、去重二选一、删除文档、PATCH 元数据、PUT 文档 ACL。闸姿态**随该入口权限码**：`requirePermission` 系（PATCH 元数据 / PUT 文档 ACL / 去重二选一）始终查；`WhenEnforced` 系随 `AUTH_ENFORCE`（关则不查，**未改仓库默认**）。成员解析经 `createDocumentRoutes({ resolveKbMember })` 注入，默认查 `kb_members`；super_admin 旁路；拒绝 403 非成员文案（`tests/acl/doc-write-kb-member-gate.test.ts`）。**残余**：闸须先取到文档行才知其 KB，持码非成员仍可经 404 与 403 的差异探测文档存在性
