...
| 最近更新 | 2026-09-20（**裁定并记明：KB 内授权判据 = 权限码 + 成员资格，`kb_members.role` 不参与运行时闸**；全仓唯一读取该列的 SQL 只服务成员列表；ADR-045 `:959`/`:965` 与 `prds/05-api:168-169` 的 role 判据记为**须 ADR 销账的文本债**，未改源码、未改 `prds/00–11`）；2026-09-20（路径只有 `:docId` 的文档写入口 10 个 + 读入口 5 个补 handler 级 KB 成员闸 `auth/doc-scope.ts`：姿态随该入口权限码，super_admin 旁路；新增 `tests/acl/doc-write-kb-member-gate.test.ts` / `doc-read-kb-member-gate.test.ts`）；2026-09-19（剧本 E4 去重冲突 `resolve` 端点；ADR-046 签字包读时派生；O4 双仓 `tenantId` 契约闸；AA1 保文档断言）；2026-09-17（问答 `Idempotency-Key`：同 key 不重跑、终态复用、在途 409；入库报告回读 `contextualizeL1Ok` / `contextualizeL0Fallback`；`PUT /documents/:docId/acl` 收紧时回 `reindexRequired`）；2026-09-16（文档 ACL 专用端点 `GET/PUT /documents/:docId/acl`；上传体积族级档：MD/TXT 更严 10 MiB；指标骨架补 `fallback` / `node_used` 维；`GET /ask/:requestId/final` 断线重拉终态；`ask_traces.citations` 落库） |
...
- **路径只有 `:docId` 的文档入口已补 handler 级 KB 成员闸**（`auth/doc-scope.ts` 的 `createDocMemberGate`，读写共用）：中间件只能从 path 取 `:kbId`，`/documents/:docId*` 这类入口原先只验权限码，持码非成员可跨库写、也可跨库读。**写面 10 个**（重跑入库、审批通过、驳回、扫描入队、lifecycle、替代、去重二选一、删除文档、PATCH 元数据、PUT 文档 ACL，2026-09-20）+ **读面 5 个**（`GET /documents/:docId`、`…/acl`、`…/ingest-jobs`、`…/chunks`、`…/chunks/:chunkId`，同日补）。闸姿态**随该入口权限码**：`requirePermission` 系（PATCH 元数据 / PUT 文档 ACL / 去重二选一 / 两个分片读）始终查；`WhenEnforced` 系随 `AUTH_ENFORCE`（关则不查，**未改仓库默认**）。顺序上成员闸是外层，先于部门 / `aclPrincipals` 第二层闸。成员解析经 `createDocumentRoutes({ resolveKbMember })` / `createChunkRoutes({ resolveKbMember })` 注入，默认查 `kb_members`；super_admin 旁路；拒绝 403 非成员文案（`tests/acl/doc-write-kb-member-gate.test.ts` · `tests/acl/doc-read-kb-member-gate.test.ts`）。**残余**：闸须先取到文档行才知其 KB，持码非成员仍可经 404 与 403 的差异探测文档存在性
- 成员路由：列表 / 邀请 / **PUT 改 role** / 删除；始终 `member.manage` + KB 成员闸。**无** `allowedDocIds`。建库会写入首位库管 `role=admin`
- **KB 内授权判据 = 权限码 + KB 成员资格；`kb_members.role` 不参与运行时闸**（2026-09-20 裁定）：依据 ADR-051 §决策 2（`prds/11-decisions/00-adr-index.md:1352`「运行时以有效码为准；`platform_role` / `kb_members.role` 为模板锚点」）、ADR-035 状态行（`:480`「角色列降级为模板锚点」）、安全 PRD §3.2 焊死（`prds/09-security/01-auth-acl-compliance.md:102`/`:104`「以码为准」）与文本内唯一裁决序（同文件 `:76`「冲突：catalog/码表 > 旧文案」）。结构与源码一致：`ResolveKbMember` 回 `boolean`、`lookupKbMembership` 缓存 `Map<string, boolean>`、`canAccessKbScoped` 入参无 role；**全仓唯一读取该列的 SQL 只服务成员列表**（`services/members.ts:98`）。**文本债**（销账须 ADR，本仓不改 `prds/00–11`）：ADR-045 `:959`/`:965` 与 `prds/05-api/01-http-api-hono.md:168-169` 的 role 判据未被任何后续 ADR 点名销账。裁定与证据链：`.scratch/kb-role-vs-code/issues/04-dec-role-gate-ruling.md`
...
- **`POST /documents/:docId/dedupe-conflicts/:chunkId/resolve`**（剧本 E4）：body `{ winner: 'this' | 'other' }`；权限 `doc.editor`（与 `PATCH /documents/:docId`、`PUT …/acl` 同码同姿态，**不**新增权限码）；仅 `dedupe_status='pending_review'` 可处理（否则 400；不存在 404）；**只动 `chunks` 两列**，`winner='this'` 清 `duplicate_of` 并回 `reindexRequired: true` —— **不代跑 reindex**（重新入库须持 `doc.reindex`，本端点不做权限升级）；**不碰对方文档**（`services/dedupe-conflict.ts` · `tests/ingest/dedupe-conflict-resolve.test.ts`）
- **`GET /api/v1/knowledge-bases/:kbId/ingest-report`**：成员闸 + `doc.view` WhenEnforced；返回该库已落库行（空列表 200；缺库 404）；只写真事（chunkCount / 文档内 dropped / **跨 doc dropped + 冲突对（`action` ∈ `skip_index|pending_review`，入审时带 `heldChunkId`）** / **跨文档去重率** / **contextSource（l0 / l0_fallback / l1_llm，白名单原样回读）** / 双就绪 / 对账计数）；**无** Hit@k / doc 级路径（`routes/ingest-report.ts` · `tests/ingest/ingest-report-http.test.ts`）
...
- 文档 `ownerDeptId` / `visibilityLevel` **已落库可回读**（`PATCH /documents/:docId` · complete body 可选同写 · `doc.editor` 始终验码 **＋ KB 成员闸**；migration `0007`）
...
- 文档级 `aclPrincipals`（可空用户 uuid 数组）：null=KB 成员可读；`[]`=非超管不可读；列表 / 详情 / chunks / `loadCorpusFromDb` 在部门滤之后同滤；不跟 `DEPT_ACL_ENFORCE`；超管 bypass；ES mapping/bulk 写 keyword 数组（null 不写、`[]` 写哨兵 `__acl_none__`，因 exists 不认空数组）；已有索引 PUT `_mapping` 补 keyword；查询期非超管 should（缺字段可读 ∪ term userId）；**专用入口 `GET/PUT /api/v1/documents/:docId/acl` 已落**（PRD 05 §2.4；GET 与详情共用同一可见性判定，PUT 用 `doc.editor` 且刻意不叠**可见性闸**（`docReadDenied`）以免 `[]` 文档无人能修，但**写与读都仍须过 KB 成员闸**（见「鉴权与权限」节；成员闸是外层，与可见性闸不是同一道））；**收紧须 reindex**（功能表 §5.5 · ADR-009 决策 4）：PUT 收紧时回 `reindexRequired=true` 并写 `doc_acl_tightened` 日志，**不自动入队 reindex**（缺「激活 version」表示，与孤儿清理同一前置）；索引滞后**不构成泄漏**（稀疏命中只保留 PG 语料内 id，语料过 `filterDocsForAclPrincipals`；`tests/acl/acl-tighten-index-lag.test.ts`）；**≠** 角色 principal / 默认开强制；`tests/acl/documents-acl-endpoint.test.ts`
...
- **签字包只读回填**：`GET …/settings` 的 `qualitySnapshot.gatePackageId` / `effectiveAt` **从 `eval_runs` 读时派生**（ADR-046 四要素之四「KB 配置快照绑定 `eval_runs`」+ ADR-061 双轨）：合格 = `run_type=golden_2x2` ∧ `status=succeeded` ∧ `retrieve_mode=live` ∧ `signoff_eligible ∈ {'1','true'}`，取 KB 内**最近一条**；无合格者保持 `null`（不臆造、不回落 env、不代签）。`effectiveAt` = 该行**已记录的创建时间**（不新增「签字时刻」字段）。`tauClaim` **仍**取 `TAU_CLAIM`（ADR-007 唯一源）。**RACI 人签是文件产物**（`fixtures/l1/RACI.md`）不在库里，故不构成过滤条件 —— 本派生只指向「工程合格包」，**≠** 业务人签。SQL 谓词未经真 PG 验证（`services/eval-runs.ts` · `tests/eval/signoff-package-derive.test.ts`）
...
| 按 `requestId` 断线重拉 | **终态回读已落**：`GET /ask/:requestId/final`（成员闸同审计口；`ready=true` 时 `response` 与在线 `data-ask-final` 同形，零引用按 status/reason 推出，verified 轮引用取自落库 `citations`；不可同形 → `ready=false` + message；无 trace → 404）。审计口 `GET /ask/:requestId` 语义不变（仍**不是** AskResponse 重放）；**无** 起始标记（trace 仍在 finalize 后写），故「还在跑」与「不存在」在 API 层不可辨 —— 该残余面改由 **`Idempotency-Key`（PRD §2.7 铁律 6）**承接，见下 |
| 问答幂等 | **已落**（PRD 05-api §1 + §2.7 铁律 6）：`POST …/ask` 读 `Idempotency-Key` → Redis `sr:ask:idem:{tenant}:{user}:{kb}:{key}`（TTL 600s；`services/redis.ts` 共享连接）；同 key 已 finalize → 复用同一 `requestId` 的终态 DTO（与 `/final` 同形，流式写 `data-ask-final`）；未 finalize → 409 `CONFLICT` + `details.requestId` / `status='in_flight'`；不可同形 → 409 `not_replayable`；跑图抛错释放 claim；不带 header 行为不变；**无** env 开关。**不做** SSE 真重连 attach / web 闭环 |
...
| 上传 MIME / checksum / 体积档 | `gates/upload-media.ts` · `gates/upload-size.ts` · `services/storage.ts`（`effectiveMaxUploadBytes` 按族取档）· `ingest-complete-pending.ts` · `tests/ingest/upload-media.test.ts` · `tests/ingest/complete-media.test.ts` · `tests/ingest/upload-size-tier.test.ts` · `tests/env/defaults.test.ts` |
