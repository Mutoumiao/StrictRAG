...
- **`GET /api/v1/knowledge-bases/:kbId/doc-types`**：始终 `requireKbMember`；只回 `{ items: [{ code, label }] }`（`label` 暂等于 `code`）；空枚举 `items: []`；**不**回 τ（`tests/ask/http-doc-types.test.ts`）；设置 GET 仍要 `kb.config.write`
