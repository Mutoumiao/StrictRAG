...
| 最近更新 | 2026-09-20（**裁定并记明：KB 内授权判据 = 权限码 + 成员资格，`kb_members.role` 不参与运行时闸**；全仓唯一读取该列的 SQL 只服务成员列表；ADR-045 焊死 #1 / #3 与 API PRD 删除判据中的 role 措辞记为**须 ADR 销账的文本债**，未改源码、未改 `prds/00–11`）；2026-09-20（路径只有 `:docId` 的文档写入口 10 个 + 读入口 5 个补 handler 级 KB 成员闸 `auth/doc-scope.ts`：姿态随该入口权限码，super_admin 旁路；新增 `tests/acl/doc-write-kb-member-gate.test.ts` / `doc-read-kb-member-gate.test.ts`）；2026-09-19（剧本 E4 去重冲突 `resolve` 端点；ADR-046 签字包读时派生；O4 双仓 `tenantId` 契约闸；AA1 保文档断言）；2026-09-17（问答 `Idempotency-Key`：同 key 不重跑、终态复用、在途 409；入库报告回读 `contextualizeL1Ok` / `contextualizeL0Fallback`；`PUT /documents/:docId/acl` 收紧时回 `reindexRequired`）；2026-09-16（文档 ACL 专用端点 `GET/PUT /documents/:docId/acl`；上传体积族级档：MD/TXT 更严 10 MiB；指标骨架补 `fallback` / `node_used` 维；`GET /ask/:requestId/final` 断线重拉终态；`ask_traces.citations` 落库） |
...
- **KB 内授权判据 = 权限码 + KB 成员资格；`kb_members.role` 不参与运行时闸**（2026-09-20 裁定）：依据 ADR-051 §决策 2（「运行时以有效码为准；platform_role / `kb_members.role` 为模板锚点」）、ADR-035 状态行（「角色列降级为模板锚点」）、安全 PRD §3.2 焊死（「以码为准」）与同文件「冲突：catalog/码表 > 旧文案」的裁决序；逐条行号见 `.trellis/spec/api/backend/auth-authorization.md` 的同名小节。结构与源码一致：`ResolveKbMember` 回 `boolean`、`lookupKbMembership` 缓存 `Map<string, boolean>`、`canAccessKbScoped` 入参无 role；**全仓唯一读取该列的 SQL 只服务成员列表**（`services/members.ts`）。**文本债**（销账须 ADR，本仓不改 `prds/00–11`）：ADR-045 焊死 #1 / #3 与 API PRD 删除判据中的 role 措辞，未被任何后续 ADR 点名销账；裁定与证据链落在本次 wayfinder 图 kb-role-vs-code 与上述 spec 小节
