...
| 最近更新 | 2026-09-16（文档 ACL 专用端点 `GET/PUT /documents/:docId/acl`）；上传体积族级档：MD/TXT 更严 10 MiB；指标骨架补 `fallback` / `node_used` 维；`GET /ask/:requestId/final` 断线重拉终态；`ask_traces.citations` 落库） |
