...
| 最近更新 | 2026-09-20（路径只有 `:docId` 的文档写入口 10 个 + 读入口 5 个补 handler 级 KB 成员闸 `auth/doc-scope.ts`：姿态随该入口权限码，super_admin 旁路；新增 `tests/acl/doc-write-kb-member-gate.test.ts` / `doc-read-kb-member-gate.test.ts`）；2026-09-19（剧本 E4 去重冲突 `resolve` 端点；ADR-046 签字包读时派生；O4 双仓 `tenantId` 契约闸；AA1 保文档断言）；2026-09-17（问答 `Idempotency-Key`：同 key 不重跑、终态复用、在途 409；入库报告回读 `contextualizeL1Ok` / `contextualizeL0Fallback`；`PUT /documents/:docId/acl` 收紧时回 `reindexRequired`）；2026-09-16（文档 ACL 专用端点 `GET/PUT /documents/:docId/acl`；上传体积族级档：MD/TXT 更严 10 MiB；指标骨架补 `fallback` / `node_used` 维；`GET /ask/:requestId/final` 断线重拉终态；`ask_traces.citations` 落库） |
