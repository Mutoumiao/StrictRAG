...
| 最近更新 | 2026-09-10（reindex 对卡在 OCR 闸的文档入队 ocr；**不是**自动全库 / **≠** 真引擎） |
...
- **B12 / ADR-053 最小闭环**：`chunk_strategy_definitions` + `kb_chunk_strategies` 为 catalog 权威；写入闸仍 **`IMPLEMENTED` 仅 `structure_paragraph`**
  ...
  - complete/reindex：按 for-upload available 计数（仅 1 个可自动；≥2 未选 400）；写入策略码 + `chunk_strategy_params` 快照；reindex 对 `needs_review` / 非 utf8 `needs_ocr` 入队 `ocr`（其余仍 `chunk`；**不是**自动全库）
