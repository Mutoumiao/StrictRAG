...
| 最近更新 | 2026-09-15（settings 类型分区 catalog：code/label/sort/enabled；成员 GET 只回启用项真 label） |
...
- `PATCH /documents/:docId`：部门 / 可见级 / **`docType`**（须属于该 KB **启用中** 的类型枚举，否则 400；空启用列表只能清 null；停用码不得新标）/ **生效区间**（`effectiveFrom`/`effectiveTo` 本地时间串，omit 不改、null 清除，合并后 from>to → 400；不改 lifecycle、不入队）；列表项含 `docType` / 窗口 / **替代边**（`supersedesDocId` / `supersededByDocId`，缺省 `null`）
...
| 项 | 说明 |
|----|------|
| 知识库设置 admin 全量 UI（分片策略弹窗 / KB 模型绑定写 UI） | API：**docTypes / docTypeItems + mode 闸 + KB 绑定读路径已接线**；类型分区 admin 已接；paramSchema / KB 绑定写 UI 可 defer |
