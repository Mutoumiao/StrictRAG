...
| 最近更新 | 2026-09-14（DELETE 响应 archived + purgeEnqueued；INGEST_STAGES 含 purge） |
...
- **入库报告**：`IngestReportItemSchema`（`.strict()`；对账可空；拒跨 doc / Hit@k）（`ingest/ingest-report.contract.ts` · `tests/ingest/ingest-report-contract.test.ts`）
