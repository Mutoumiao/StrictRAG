...
| 入库报告完整语义 | 跨 doc `skip_index`（字 3-gram Jaccard≥0.9）+ 冲突对（可含 `pending_review` + `heldChunkId`）+ contextSource + **跨文档去重率** 已落；**无**处置 / 聚合指标 / 审阅面 / 生产 LSH / Hit@k / 「高度重复」阈值提示 |
