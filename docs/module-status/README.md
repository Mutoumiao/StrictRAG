...
| **观测 / 评测** | **可联调**（进程内 metrics/tracer；L1 gold60 + OPS-1 live；2026-08-14 live 全量 ×2 `signoffEligible=true`；ADR-046 快照可绑定；coverage=0 **≠** 业务 PASS；`eval_runs` 可选落库（L1 `golden_2x2` / L2 `session_multiturn`）；**mock 禁签字**；**L2 题面 + 工程 runner + 可选 persist 已落**（`fixtures/l2/` + `run-l2-golden`，`L2_PERSIST_EVAL` 默关）；**有账本 ≠ 准出**；**rewrite 图边已落、默认关**；**显式回溯加深部分**；**文档回溯检索加码部分**；**库外文档回溯抑制部分**；**L3 打点部分**（六 counter；**无**自动熔断 / **无**面板）） | `api` | `db` | [api](./api.md) · [db](./db.md) · [live profile](../ops/live-retrieve-profile.md) |
