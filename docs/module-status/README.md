...
| **鉴权 / 访问控制（ACL）** | **可联调**（临时双 JWT；KB 成员 + 权限码；B5 部门骨架；文档部门字段可改（含 complete / 列表列 / 下拉）；grant 表可存可配；`DEPT_ACL_ENFORCE` **默认关**，开时精确 ∪ 祖先 + 精确 grant；超管可绕过；列表同滤；可关继承（env + KB 覆盖）；设置页可标 `dataClass`（**≠**解禁）；`AUTH_ENFORCE` 默认关；**≠** 全文隔离 / **≠** ES 查询期） | `api` | `admin-catalog` · `admin` · `web` · `contracts` · `db` | [api · 鉴权](./api.md) · [admin-catalog](./admin-catalog.md) |
