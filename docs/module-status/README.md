...
| **模型网关** | **可演示**（供应商 CRUD + **platform** 绑定；运行时 env+platform+**KB 读覆盖**；generate fallback **opt-in** 切链；**无** `GENERATE_MIN_NODES`；**无** KB 绑定 HTTP 写；admin KB 写 UI 延后；**≠** 生产多活） | `api` | `admin` · `contracts` · `db` | [api](./api.md) · [admin](./admin.md) |
