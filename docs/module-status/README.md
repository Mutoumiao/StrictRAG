...
| **鉴权 / 访问控制（ACL）** | **可联调**（临时双 JWT；KB 成员 + 权限码；B5 部门骨架；文档部门字段可改（含 complete / 列表列 / 下拉 / 本地筛）；grant 表可存可配（部门下拉）；`DEPT_ACL_ENFORCE` **默认关**，开时精确 ∪ 祖先 + 精确 grant；超管可绕过；列表同滤；可关继承（env + KB 覆盖 + 设置页勾选，未改不写回）；设置页可标 `dataClass`（**≠**解禁）；`AUTH_ENFORCE` 默认关；**≠** 全文隔离 / **≠** ES 查询期） | `api` | `admin-catalog` · `admin` · `web` · `contracts` · `db` | [api · 鉴权](./api.md) · [admin-catalog](./admin-catalog.md) |
...
| **观测 / 评测** | **可联调**（进程内 metrics/tracer；L1 gold60 + OPS-1 live；2026-08-14 live 全量 ×2 `signoffEligible=true`；ADR-046 快照可绑定；coverage=0 **≠** 业务 PASS；`eval_runs` 可选落库（L1 `golden_2x2` / L2 `session_multiturn`）；**mock 禁签字**；**L2 题面 + 工程 runner + 可选 persist 已落**（`fixtures/l2/` + `run-l2-golden`，`L2_PERSIST_EVAL` 默关）；**有账本 ≠ 准出**；**rewrite 图边已落、默认关**；**显式回溯加深部分**；**文档回溯检索加码部分**；**库外文档回溯抑制部分**；**四态派生部分**；**L3 打点+告警部分**（六 counter + 主题投诉 + `l3_guard_alert_total` 含 `l2_stale`；**无**自动熔断 / **无**面板 / **≠**准出）） | `api` | `db` | [api](./api.md) · [db](./db.md) · [live profile](../ops/live-retrieve-profile.md) |
