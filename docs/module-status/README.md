...
| **契约 / Schema 基座** | **可联调**（P1/S2 + B1–B6 + **B12 策略码 / IngestJobData**；`eval_runs` 可落；`ingest_jobs` worker 最小 stage 写；`permission_definitions` 启动字典；同 doc Redis 锁最小） | `contracts` / `db` | 全部业务包 | [contracts](./contracts.md) · [db](./db.md) |
