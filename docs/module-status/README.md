...
| **观测 / 评测** | **可联调**（进程内 metrics/tracer；L1 gold60 + OPS-1 live；**P2 底线** gold-questions HTTP + POST eval/runs 入队 + worker sr-eval + admin /eval；L1 有 `expectedDocIds` 时报告 Hit@k（**不**进签字公式）；L1 按 minSupport 离线扫 τ 得 tau*（**不**写 `TAU_CLAIM` / **不**进签字公式）；**L2 归档底线** session_multiturn 入队 + 多轮窗 + 工程 signoffEligible；coverage=0 **≠** 业务 PASS；**mock 禁签字**；**有账本 ≠ 准出**；**rewrite 图边已落、默认关**；无合格归档不得写产品默认开；**L3 打点+告警+进程内熔断部分**（三 kind 闩后关 rewrite；`rewrite_dogfood` 不熔；**无**写 env / **无**面板 / **≠**准出）） | `api` | `worker` · `admin` · `db` | [api](./api.md) · [worker](./worker.md) · [admin](./admin.md) · [db](./db.md) · [live profile](../ops/live-retrieve-profile.md) |
