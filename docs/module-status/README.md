...
| **观测 / 评测** | **可联调**（进程内 metrics/tracer；L1 gold60 + OPS-1 live；2026-08-14 live 全量 ×2 `signoffEligible=true`；ADR-046 快照可绑定；coverage=0 **≠** 业务 PASS；`eval_runs` 可选落库；**mock 禁签字**；**L2 题面 + 工程 runner 已落**（`fixtures/l2/` + `run-l2-golden`，注入测绿）；**≠** L2 准出；**rewrite 图边已落、默认关**；**显式回溯加深部分**；**L3 打点部分**（四 counter；**无**自动熔断 / **无**面板）） | `api` | `db` | [api](./api.md) · [db](./db.md) · [live profile](../ops/live-retrieve-profile.md) |
