...
| **会话外壳** | **可演示**（列表 / 历史回放；**rewrite 图边已落、默认关**；**显式回溯加深部分**；**文档回溯检索加码部分**；**库外文档回溯抑制部分**；**≠** 准出 / **≠** 对外连续追问） | `api` | `web` · `db` · `contracts` | [api](./api.md) · [web](./web.md) |
| **鉴权 / 访问控制（ACL）** | **可联调**（临时双 JWT；KB 成员 + 权限码；**B4-W** 从 DB `codes_json` hydrate；B5 部门骨架；`AUTH_ENFORCE` 默认关；**无** DEPT_ACL 检索强制） | `api` | `admin-catalog` · `admin` · `web` · `contracts` · `db` | [api · 鉴权](./api.md) · [admin-catalog](./admin-catalog.md) |
...
| **观测 / 评测** | **可联调**（进程内 metrics/tracer；L1 gold60 + OPS-1 live；2026-08-14 live 全量 ×2 `signoffEligible=true`；ADR-046 快照可绑定；coverage=0 **≠** 业务 PASS；`eval_runs` 可选落库（L1 `golden_2x2` / L2 `session_multiturn`）；**mock 禁签字**；**L2 题面 + 工程 runner + 可选 persist 已落**（`fixtures/l2/` + `run-l2-golden`，`L2_PERSIST_EVAL` 默关）；**有账本 ≠ 准出**；**rewrite 图边已落、默认关**；**显式回溯加深部分**；**文档回溯检索加码部分**；**库外文档回溯抑制部分**；**L3 打点+告警部分**（六 counter + `l3_guard_alert_total`；**无**自动熔断 / **无**面板 / **≠**准出）） | `api` | `db` | [api](./api.md) · [db](./db.md) · [live profile](../ops/live-retrieve-profile.md) |
