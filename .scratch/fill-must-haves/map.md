...
- **断线按 requestId 重拉终态（工单 92 的前置）**：`ask_traces` **没有 citations 列**，`graphTrace` 只留 4 个键，且 `generateSystemPrompt()` 要求 citations 走独立 JSON 数组、**不**要求答案嵌 `[chunkId]` token —— 所以不能从 answer 文本反推引用。要先落 citations 持久化（新列 + `db:generate` 迁移 + execute 写入），否则重拉只能吐出「answered 却零引用」的假终态
