...
- 顺序：本图 P2 语义已收官（第三批）；P2.5 出口走 **L2 归档准出**（工程路径 [L2 归档底线](./issues/14-l2-archive-floor.md) 已齐，人签仍图外）。鉴权/成员余量已齐。入库报告最小闭环已齐。库选择器只列成员库已齐。admin 顶栏当前 KB 选择器已齐。web 下拉换 ui 关闭列表已齐。启动引导超管已齐（env 创建，不是页）。其余 P2 半接线留雾。P3a 仍等该出口。P3b 可在 P2 后并行，但不抽走下一裁定。
