...
- **回归债（脆弱测例）**：`apps/api/tests/acl/kb-member-gate.test.ts > POST members without auth → 401` 在全量跑里**间歇超时**（`testTimeout` 5s；单跑实测 3.06s，`createApp` 动态导入 + 并行 worker 抢 CPU）。已在改动前基线上复跑确认同样失败 → **非本次引入**。修法很小（给该 `it` 加 `{ timeout: 30000 }` 或预热导入），但它是「门禁红一条」的持续来源，需另开小工单
