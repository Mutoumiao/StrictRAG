...
- 剩余必须具备（三批已收齐 94/95/96/99/100/101）：`dedupe_cross_doc_rate` **已齐（95）**· metrics `fallback` / `node_used` 维 **已齐（94）**· MD/TXT 更严体积档 **已齐（96）**· 文档 ACL 端点 **已齐（100）**· 真 L1 contextualize **已齐（101，默认关）**。仍后批：admin 文档 ACL 编辑面（选人来源待定）· 在线编写完整体验其余（BlockNote / editor-draft，落点未冻）· 魔数嗅探（**PRD 无依据，属加固**）· 入库报告 L0 vs L1 Hit@k（**前置：同文档两份可检索数据的载体**）· 「高度重复」提示阈值（**前置：数据 PRD 给阈值**）· reindex-on-tighten（先写清「不是安全洞」）· dense 单路负向夹具（B2-3）· `pending_review`（**前置：落点 / 人工端点 / KB 策略位三处先冻**）· QUAL-G3 `gold.yaml` 审核闸（**功能表无此行**）· 成员 `allowedDocIds` 面 · worker metrics 出口（`contextualize_*` 要变真计数器得先有它）· `Idempotency-Key`（**前置：先研究**）· `drizzle/meta` 基线缺失（`db:generate` 仍不可用）
