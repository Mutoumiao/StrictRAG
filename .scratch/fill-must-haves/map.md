...
- **回归债（脆弱测例）**：全量 `pnpm test` 里会**间歇** `Test timed out in 5000ms` 的是「测试内 `await import(重模块)`」那类：`apps/api/tests/acl/kb-member-gate.test.ts > POST members without auth → 401`（单跑实测 3.06s）与 `apps/api/tests/obs/tracer.test.ts > chitchat 路径留下 route+finalize`（单跑实测 1.49s）—— 并行 worker 抢 CPU 时越过 5s 默认阈值。已用 `git stash`（含未跟踪）在改动前基线上对照：基线 `--filter api test` 单跑同样只有 `kb-member-gate` 超时；基线全量为绿。修法很小（给这两个 `it` 加 `{ timeout: 30_000 }` 或改预热导入），但它是「门禁红一条」的持续来源，需另开小工单
