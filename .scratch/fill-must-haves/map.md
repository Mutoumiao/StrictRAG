...
- 顺序：本图 P2 语义已收官（第三批）；P2.5 出口走 **L2 归档准出**（工程路径 [L2 归档底线](./issues/14-l2-archive-floor.md) 已齐，人签仍图外）。鉴权/成员余量、入库报告、库选择器、admin 顶栏、web 关闭列表、启动引导超管、写路径锁超管全码、修改日志、三平面配额、失败 Webhook 已齐。P3b 可动手最小闭环已齐。P4 可动手代码真空已尽。P5 OCR 开闸与历史 `needs_ocr` 运营重跑已齐（默认关）；**P5 可动手代码真空已尽**。暂停已解除；在线编写最小闭环已齐（无 BlockNote）。生效区间最小闭环已齐。替代联动最小闭环已齐。删除与 purge 最小闭环已齐。文档类型成员面最小闭环已齐。上传 MIME 白名单最小闭环已齐。类型分区 CRUD 最小闭环已齐。上传表单标部门最小闭环已齐。KB 消费绑定最小闭环已齐。同 KB 跨文档去重最小闭环已齐。L0 模板 / contextMode 单控件最小闭环已齐。反馈回流黄金集最小闭环已齐。本轮裁定三张串行：[禁自审四眼](./issues/86-no-self-approve-min.md) **已齐** → [角色树状勾选](./issues/87-role-permission-tree-min.md) **已齐** → [分片策略服务端修改日志](./issues/88-chunk-strategy-audit-min.md) **已齐**。新一批三张串行：[文档策略快照只读](./issues/90-doc-strategy-snapshot-readonly.md) **已齐** → [citation chunk 级去重](./issues/91-citation-dedupe.md) **已齐** → [断线按 requestId 重拉终态](./issues/92-ask-requestid-replay.md) **已齐**（含前置：`ask_traces.citations` 落库 + migration `0017`）。本批（[裁定 98](./issues/98-after-96-order.md)）三张串行：[lint 门禁清零](./issues/99-lint-zero-warnings.md) **已齐** → [文档 ACL 端点](./issues/100-doc-acl-endpoint.md) **已齐** → [真 L1 contextualize](./issues/101-real-l1-contextualize.md) **已齐**。另插入一张清理债：[admin 脆弱测例超时热修](./issues/102-admin-test-timeout.md) **已齐**。本批（[裁定 103](./issues/103-after-101-order.md)）三张串行：[问答 `Idempotency-Key`](./issues/104-ask-idempotency-key.md) **已齐** → [入库报告补 contextualize 两计数](./issues/105-ingest-report-contextualize-counts.md) **已齐** → [文档 ACL 收紧索引一致性](./issues/106-acl-tighten-index-consistency.md) **已齐**。**本批已收官**；下一步需再裁定。**孤儿清理与签字包链已撤销排序**（各缺一个前置：见 Notes 尾）。P3a 仍等 L2 人签。不默认开 `DEPT_ACL_ENFORCE`。不加角色 principal。不默认开 OCR。不自动全库重跑。
- **本轮收口门禁（2026-09-17）**：`pnpm test` **11/11**（api 137 files / 878 passed + 3 skipped · worker · admin · contracts · web · db · admin-catalog 全绿；**零超时**）· `pnpm check-types` **8/8** · `pnpm lint` **8/8 零 warning**。各工单 Answer 里的「见地图收口数字」指本行。
- **本轮收口门禁（2026-09-16）**：`pnpm test` **11/11**（api 134 files / 856 passed + 3 skipped · contracts 26/215 · worker 34/165 · admin 34/169 · web 18/55 · db 10/29 · admin-catalog 1/11；**零超时**）· `pnpm check-types` **8/8** · `pnpm lint` **8/8 零 warning**。各工单 Answer 里的「见地图收口数字」指本行。
...
- [真 L1 contextualize 最小闭环](./issues/101-real-l1-contextualize.md) — `contextualize-http.ts`（OpenAI 兼容 chat，temp=0，PRD §4.1 冻结模板；空/超长/非 2xx/畸形一律抛）；`INGEST_CONTEXTUALIZE_MODE` **默认 off**（无真 Gateway 时 on 只会全量假 `l0_fallback`）；逐块调用，成功才 `l1_llm`、任一块回退即 `l0_fallback`（不阻断、不改正文）；`CONTEXT_SOURCES` 放开 `l1_llm`。**未做**：per-chunk checkpoint；`contextualize_*` 只作日志字段（**worker 无 metrics 出口**，未假造计数器）；**未验证真 Gateway**。
- [裁定真 L1 contextualize 后下一步](./issues/103-after-101-order.md) — 本批串行三张：`Idempotency-Key`（铁律 6）→ 入库报告补 contextualize 两计数 → 文档 ACL 收紧索引一致性。**worker metrics 出口明确不开端口**（功能表 `:34` / `:206` 两处把「Worker 对外 HTTP」写进禁止列；`prds/10` §2 对 worker 观测只写「建议具备导出、实现可选」）——`contextualize_l1_ok` / `l0_fallback` 的正解是落进**入库报告**（`prds/04` §5.2「指标（入库报告必出）」逐字点名），不是 scrape 端点。**`allowedDocIds` 下调为雾**：语义已冻（ADR-009 决策 3 / 05-api `:539` / ES PRD §4.4 / 04-pipelines「状态字段语义」）但 ES PRD 自标「debug / 评测 / 迁移 · **可选**」、客户端禁传、仓库无任一此类调用者 → 现在实现等于造无生产者的半接线；**准入条件**：先指名真实生产者或书面判定为保留码。**成员 `allowedDocIds` 写面确认不做**：05-api §2.2 逐字是 `PUT …/members/:userId # body: { role: read|write|admin }`，功能表 §5.2 成员行的「null allowedDocIds = 成员全库」说的是取值语义，工单 19 裁定与 `packages/contracts/tests/ask/contract.test.ts:113` 保持。**ACL 收紧不做自动 reindex**（与孤儿清理同一前置）。**admin 文档 ACL 编辑面**（功能表 §4.3 无此行动作）与 **`editor-draft` / BlockNote**（功能表 §5.2 有 `editor-draft` + `submit-approval` 行，P2.x；BlockNote 不在冻结技术栈表）留雾，后者是下批首选的**决定工单**。研究：本轮新写 [research-idempotency-key.md](./research-idempotency-key.md) · [research-metrics-worker.md](./research-metrics-worker.md) · [research-drizzle-meta-baseline.md](./research-drizzle-meta-baseline.md)。
- [问答 `Idempotency-Key` 最小闭环](./issues/104-ask-idempotency-key.md) — PRD 05-api §2.7 **铁律 6** + §1 写操作总则 + 04-pipelines §8；落点与 TTL 已冻（`ask:idem:{key}` 10m → 本仓键 `sr:ask:idem:{tenant}:{user}:{kb}:{key}`，TTL 600s，`services/redis.ts` 与队列共享连接）。命中已 finalize → 复用 `toAskFinal`（与 `/final` **同形**）；在途 → **409 `CONFLICT`** + `details.requestId`（`status='in_flight'`）；不可同形 → 409 `not_replayable`；**跑图抛错释放 claim**（不得留 10m 假在途）；幂等短路在**限流之前**（重试不吃配额）；不带 header 行为逐位不变、**无 env 开关**；`AskRequestSchema` 不改（键是请求头，`.strict()`）；OpenAPI 登记可选 header。**不做**：SSE 真重连 attach · web 客户端闭环 · 命中计数指标 · 起始标记进 `ask_traces` · 进程内 Map（多副本违反硬性「不得」）· `x-request-id` 顶替。**未验证**真 Redis 集群多副本（单测用内存 store）。
- [入库报告补 contextualize 两计数](./issues/105-ingest-report-contextualize-counts.md) — `prds/04` §5.2「指标（入库报告必出）」四行末两行（`contextualize_l1_ok` / `contextualize_l0_fallback`）此前只写 Pino 日志；现落库（migration `0019`，两列 integer **无默认**）+ contracts DTO + api 原样回读 + admin 行展示「L1 成功 N · L0 回退 M」。**口径与 `contextSource` 同调**：文档请求 L1 而本轮未实际调用时整轮计回退（不得出现「情境 `l0_fallback` · L0 回退 0」的自我矛盾行）；`l0_template` 两计数 0；后阶段（embed / es_index）不得复写已记录值；未记录一律明说未记录（不补 0）。
- [文档 ACL 收紧的索引一致性最小闭环](./issues/106-acl-tighten-index-consistency.md) — 功能表 §5.5「收紧须 reindex」+ ADR-009 决策 4 + ES PRD §4.3；`coverage 02-acl` 的 B2-2 原写「reindex-on-change 未做」。做：`aclTightens(prev,next)`（新集合不再是旧集合的超集 = 有人失去可读性）+ `PutDocumentAclResponseSchema.reindexRequired`（GET 形状不变）+ 收紧时 `doc_acl_tightened` 日志。**先钉事实**：`retrieve.ts` 稀疏命中**只保留 PG 语料内 id**、语料过 `filterDocsForAclPrincipals` → **索引滞后不构成泄漏**（只是召回被稀释），故**不自动入队 reindex**（`index_version` 在 chunk 段即 `+1`，失败会指向失败版本；缺「激活 version」表示，与孤儿清理同一前置）。台账：B2-2 / B2-3 由「延后」转「部分测」（计数 37→39 / 4→2；仍欠角色码 principal / 自动 reindex / dense 反向构造）。

## Not yet specified

- **断线重拉的残余面 → 已由 `Idempotency-Key` 收口（[工单 104](./issues/104-ask-idempotency-key.md)）**：`ask_traces` 仍只在 **finalize 后**写（PRD §2.7 铁律 5），但带 `Idempotency-Key` 的同 key 重试现在**可辨**——未 finalize → 409 `CONFLICT`（不重跑图），已 finalize → 复用同一 `requestId` 的终态。**仍未做**：不加起始标记进 `ask_traces`（会污染 `status`/审计口径）；`status=ready` 但流里没有 final 的路径仍走原 error 兜底（R1），不自动重拉
- **孤儿清理（剧本 L7 / `ingest.maintenance`）**：存储边界 §2.4「必须」已冻（触发=周期 + 文档 failed、对象=单边有向量或 ES、护栏=**激活版永不删**、动作=PG 向量 + ES 双侧）。**前置未解**：`documents.index_version` 在 chunk 段就 `+1`，reindex 失败后它指向失败版本而上一个可检索版本的数据仍在 → 今天没有「当前激活 version」可断言，硬做会误删。需先开一张决定工单钉表示（列 / 表 / 由 `status=ready` 快照派生）；触发=周期另依赖调度基建。**同一前置已挡住两处自动化**：孤儿清理与 [ACL 收紧自动 reindex](./issues/106-acl-tighten-index-consistency.md)（后者本批只做外显 + 无泄漏证明）
...
- 剩余必须具备（五批已收齐 94/95/96/99/100/101/104/105/106）：`dedupe_cross_doc_rate` **已齐（95）**· metrics `fallback` / `node_used` 维 **已齐（94）**· MD/TXT 更严体积档 **已齐（96）**· 文档 ACL 端点 **已齐（100）**· 真 L1 contextualize **已齐（101，默认关）**· 入库报告必出指标四行 **全齐（94/95/105）**· `Idempotency-Key` **已齐（104）**· ACL 收紧索引一致性 **已齐最小（106）**。仍后批：admin 文档 ACL 编辑面（功能表 §4.3 无此行动作，属推定；选人来源待定）· 在线编写完整体验其余（`editor-draft` + BlockNote，功能表 §5.2 P2.x；落点未冻）· 魔数嗅探（**PRD 无依据，属加固**）· 入库报告 L0 vs L1 Hit@k（**前置：同文档两份可检索数据的载体**）· 「高度重复」提示阈值（**前置：数据 PRD 给阈值**）· 自动 reindex-on-tighten（**前置：激活 version 表示**）· dense 反向构造（B2-3 剩余）· `pending_review`（**前置：落点 / 人工端点 / KB 策略位三处先冻**）· QUAL-G3 `gold.yaml` 审核闸（**功能表无此行**）· 成员 `allowedDocIds` 面（**已确认不做**，见裁定 103）· `allowedDocIds` 收紧路径（**准入条件：先指名真实生产者**）· worker metrics 出口（**已裁定不开端口**）· `drizzle/meta` 基线缺失（`db:generate` 仍不可用，见下条）
...
- **回归债（脆弱测例）→ 已修两处**：见 [api 侧 97](./issues/97-flaky-test-timeout.md) 与 [admin 侧 102](./issues/102-admin-test-timeout.md)。根因同一：jsdom 渲染 / 测试体内 `await import(重模块)` 的集成式用例，其耗时计在**用例超时**里，并行跑抢 CPU 时越过 vitest 默认 5000ms；`apps/api` 与 `apps/admin` 已各显式设 `testTimeout: 20_000`（`apps/web` 未观察到红，故未动）。**只调超时预算，未动任何断言 / mock / 覆盖**。判定标准写进 `.trellis/spec/guides/testing.md`：**只在观察到红时才加预算**
- **工程债（迁移工具）**：取证见 [research-drizzle-meta-baseline.md](./research-drizzle-meta-baseline.md)（2026-09-17，数字已复核）。`packages/db/drizzle/meta/` 物理上只有 `0000_snapshot.json` 与 `_journal.json`；**缺 `0001`–`0019` 共 19 份快照**（旧文写的「`0001–0016`」已滞后）。死因坐实：`drizzle-kit` 只读 `meta/` 下**物理存在**的快照、**不与 `_journal.json` 交叉校验** → `generate` 退化为「从 `0000` 全量追平」（**25 张表**，非 26——`schema_meta` 已被 0000 覆盖），且 `drizzle-kit check` **假绿**。严重性：已装 `drizzle-kit@0.31.10` 的生成器**不输出 `IF NOT EXISTS`** → 一份 generate 产物会让 `migrate` 在已建表的库上直接报 `relation "…" already exists`；「禁止提交 generate 产出」是硬约束，不是整洁偏好。**零漂移已核**：26 张表 / 350 列 / 10 唯一索引，表集合与列名集合逐表一致（**类型与默认值级尚未走查**）。**无生产上线证据**（无 Dockerfile / k8s / CD；compose 只跑本地中间件；试点人签未完成）→ 允许重置类路径。**推荐 A1**（债务即停扩大）：仓外副本跑一次 `generate` 取快照 → 只留快照、改名 `0019_snapshot.json`（**不能叫 `0020`**，那是下次 generate 要写的名）、改 `id`/`prevId`、**丢弃**其 `.sql`；硬指标 = 本仓再跑 `generate` 输出 `No schema changes, nothing to migrate`。**采纳前须先做类型/默认值级人工走查**（快照描述的是 schema 视图，实库来自手写 SQL）。**不推荐 B2（压扁重编号）**：收益只是编号美观，代价是 6 处以上文档证据指针失效 + 本地库重建。**当前实践**（手写 SQL + 手写 journal 条目）已写进 db spec 与 `docs/module-status/db.md`；迁移 `0019` 即如此。
