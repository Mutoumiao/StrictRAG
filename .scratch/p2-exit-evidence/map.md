...
- **前沿已空**：工单 [14](./14-dec-doc-write-kb-membership.md)（无 `:kbId` 的文档写入口不查 KB 成员资格）已裁定为**缺口**并落地。它是本图**工作中新冒出**的决定，不是原目的地的一步。裁定依据是 ADR-035 §决策 4（点名「删文档」这类内容路径须有成员行）与 ADR-045 焊死 #1（handler 纵深），不是人签；洞口比票面大，**10 个入口**（非 2 个）。读路径同形缺口当时未动，已作为**续图** [`doc-read-member-gate`](../doc-read-member-gate/map.md) 开出去并收口（再补 5 个读入口 + 抽共享闸模块）。
...
- **读路径的同形缺口**（工单 14 的邻居，2026-09-20 已由**续图** [`doc-read-member-gate`](../doc-read-member-gate/map.md) 收口）：`GET /documents/:docId`、`GET …/acl`、`GET …/chunks`、`GET …/ingest-jobs` 同样是「路径只有 `:docId`」且不查 `kb_members`，而 ADR-035 §决策 4 明确把「读文档内容/列表」列入须成员行的内容路径。**续图已补 5 个读入口**（`chunk.view` 两个硬姿态、`doc.view` 三个随码），并把闸抽到共享模块 `apps/api/src/auth/doc-scope.ts`；本条留作指针，不再在本图处理。
